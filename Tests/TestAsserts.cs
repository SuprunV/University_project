...
namespace Project_1.Tests
{
    public abstract class TestAsserts
    {
        ...
        protected static void areEqual(object? expected, object? actual, string? msg = null) =>
            Assert.AreEqual(expected, actual, msg);

        protected static void areNotEqual(object? expected, object? actual, string? msg = null) =>
            Assert.AreNotEqual(expected, actual, msg);

        protected static void isInstanceOfType(object o, Type expectedType, string? msg = null) =>
            Assert.IsInstanceOfType(o, expectedType, msg);
        ...
        protected static void areEqualProperties(object? a, object? b, params string[] exclude)
        {
            ...
            {
                if (((IList)exclude!)?.Contains(piA.Name) ?? false) continue;
