...
namespace Project_1.Tests.Aids {
    [TestClass] public class GetRandomTests : TypeTests {
        ...
        public void ValueTest()
        {
            ...
            areNotEqual(x?.UniID, y?.UniID, nameof(x.UniID));
            areNotEqual(x?.FirstName, y?.FirstName, nameof(x.FirstName));
            areNotEqual(x?.LastName, y?.LastName, nameof(x.LastName));
            areNotEqual(x?.EnrollmentDate, y?.EnrollmentDate, nameof(x.EnrollmentDate));
        }
        ...
        [DataRow(typeof(IsoSemester?), false)]
        [DataRow(typeof(IsoGender), true)]
        ...
        [DataRow(typeof(IsoSemester?), typeof(IsoSemester))]
